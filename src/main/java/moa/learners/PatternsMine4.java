...
*/
public class PatternsMine4 extends AbstractLearner implements Observer {
...
//                    double ratio = (double)countGroup/((double)countUsers/(GroupCounter.groupscounters.length-1));
            ...
            for(Map.Entry<SemiFCI, FciValue> entry : sortedByValue.entrySet()) {
                ...
                while(itItems.hasNext()){
                    ...
                    if(!window.contains(item) && !recommendations.contains(item)){  // create unique recommendations
                        recommendations.add(item);
                        if(recommendations.size() >= numberOfRecommendedItems/2.0){
                            break;
                        }
                    }
                }
        ...
        mapFciWeight = new HashMap<>();
        FCITable fciGlobal = this.incMine.fciTableGlobal;
        Iterator<SemiFCI> itGlobal = fciGlobal.iterator();
        while(itGlobal.hasNext()){
            SemiFCI fci = itGlobal.next();
            if(fci.size() > 1){
                List<Integer> items = fci.getItems();
        ...
        for(Map.Entry<SemiFCI, FciValue> entry : sortedByValue.entrySet()) {
            ...
            while(itItems.hasNext()){
                ...
                if(!window.contains(item) && !recommendations.contains(item)){  // create unique recommendations
                    recommendations.add(item);
                    if(recommendations.size() >= numberOfRecommendedItems){
                        break;
                    }
                }
            }
        ...
        double lcsVal = LCS.computeLongestCommonSubset(outOfWindow, recommendations);
